...
function assignUIDsToGoodTherapist(therapistUIDs)
{
	for (let i = 0; i < therapistUIDs.length; i++) {
		const therapistIndex = data_config.therapists.findIndex(elem => elem.email === therapistUIDs[i].email);
		if (therapistIndex > -1)
			data_config.therapists[therapistIndex].uid = therapistUIDs[i]._id;
	}
}
